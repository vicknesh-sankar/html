$(function () {
    ...
    $(document).on("click", "#next-item-link", function (e) {
        e.preventDefault();
        //selectItem("next");
        turnItem(1);
    });

    $(document).on("click", "#previous-item-link", function (e) {
        e.preventDefault();
        //selectItem("previous");
        turnItem(-1);
    });
    ...
    $(document).on("click","#close-showcase",function (e) {
        history.pushState('', document.title, window.location.pathname);
        refreshAfterHashChange();
        e.preventDefault();
        doSomething();
    ...
    $(document).on("click",".category h3",function (e) {
...
function selectItem(direction) {
    if (direction == "next" && $(".item-element.active").next() != undefined) {
        $($(".item-element.active").next().find(".item-link")).trigger("click");
    } else if (direction == "previous" && $(".item-element.active").prev() != undefined) {
        $($(".item-element.active").prev().find(".item-link")).trigger("click");
    }
}
...
function findElementItem(elementText) {
    $(".item-element").hide();
    $(".category").hide();
    $(".col-lg-3").hide();
    $(".item-link span").each(function (index, item) {
        if ($(item).text().indexOf(elementText) != -1)
        {
            $(item).parents(".category").show();
            $(item).parents(".item-element").show();
            $(item).parents(".col-lg-3").show();
        }
    });
}

$(document).ready(function() {
    $("#search_top").bind("keyup", function (e) {
        findElementItem($(this).val());
    });
});
